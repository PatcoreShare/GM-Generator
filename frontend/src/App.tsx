...
import { Plus, ChevronLeft, Skull, ShieldAlert, LogOut, User as UserIcon, FileText, Users, Table as TableIcon, X, Search } from 'lucide-react';
    ...
    <div className="min-h-screen bg-background text-foreground selection:bg-primary selection:text-primary-foreground">
      <header className="border-b-4 border-double border-primary/40 bg-background/80 sticky top-0 z-10 backdrop-blur-md shadow-sm">
        <div className="max-w-full mx-auto px-4 h-20 flex items-center justify-between">
          <div className="flex items-center gap-4 cursor-pointer" onClick={() => setView('list')}>
            ...
            <div>
              <h1 className="text-2xl font-black tracking-tighter fancy-heading uppercase leading-none">Archiwum WFRP</h1>
              <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-primary/60">Repozytorium Imperialne</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 mr-4 border-r border-primary/10 pr-4">
              <div className="text-right hidden sm:block">
                <p className="text-[10px] font-bold uppercase tracking-widest text-primary/60">Zalogowany jako:</p>
                <p className="text-xs font-black text-primary uppercase">{user.username} {user.role === 'admin' && '(ADMIN)'}</p>
              </div>
              <div className="bg-primary/10 p-2 rounded">
                <UserIcon className="w-4 h-4 text-primary" />
              </div>
              <Button variant="ghost" size="icon" onClick={handleLogout} className="text-primary/40 hover:text-destructive">
            ...
            {view !== 'list' && (
              <Button variant="ghost" onClick={() => setView('list')} className="text-primary/60 hover:text-primary font-bold uppercase text-xs tracking-widest">
      ...
      <main className="max-w-full mx-auto px-4 py-6">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 gap-4">
            ...
            <p className="text-primary/40 italic font-serif">Konsultowanie starożytnych zwojów...</p>
          </div>
        ) : (
                ...
                <div className="flex-1 space-y-6">
                  <div className="flex flex-col gap-4 border-b border-primary/10 pb-4">
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                      <h2 className="text-2xl font-bold fancy-heading uppercase tracking-tight">Imperialne Archiwa</h2>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      {uniqueUsers.map(u => (
                        ...
                        >
                          ...
                          <span className="font-bold uppercase tracking-widest text-xs text-primary">{u.name || 'Anonimowy Skryba'}</span>
                      ...
                      {uniqueUsers.length === 0 && (
                        <div className="col-span-full py-12 text-center border-2 border-dashed border-primary/10 rounded">
                          <p className="text-primary/40 italic">Brak wpisów stworzonych przez użytkowników.</p>
                        </div>
                      )}
                    </div>
                  ) : (
                      ...
                      {selectedOwnerId && (
                        <div className="flex items-center gap-2 mb-4">
                          ...
                          <span className="text-xs font-bold uppercase tracking-widest text-primary/40">
                            Wpisy użytkownika: <span className="text-primary">{uniqueUsers.find(u => u.id === selectedOwnerId)?.name}</span>
                          </span>
                        </div>
                      )}
                      <GeneratorList 
      ...
      <footer className="border-t-4 border-double border-primary/20 py-12 mt-20 bg-primary/5">
        <div className="max-w-7xl mx-auto px-4 text-center space-y-4">
          ...
          <p className="text-primary/40 text-xs uppercase tracking-[0.4em] font-bold">Narzędzie do Warhammer Fantasy Roleplay • Sigmar Chroni</p>
          <p className="text-primary/20 text-[10px] font-mono">ARCHIVE_PROTOCOL_v2.0.0_STABLE</p>
        </div>
      </footer>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm p-4">
          <div className="parchment-card max-w-2xl w-full p-8 space-y-8 animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center border-b border-primary/20 pb-4">
              <h2 className="text-2xl font-black fancy-heading uppercase tracking-tight text-primary">Wybierz Typ Archiwum</h2>
              <Button variant="ghost" size="icon" onClick={() => setShowTypeSelector(false)} className="text-primary/40 hover:text-primary">
                <X className="w-6 h-6" />
              </Button>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              <button 
                onClick={() => { setSelectedItem(null); setCreateType('table'); setView('create'); setShowTypeSelector(false); }}
              ...
              >
                ...
                <div className="text-center">
                  <span className="block font-black uppercase tracking-widest text-sm text-primary">Tabela</span>
                  <span className="text-[10px] text-primary/40 uppercase font-bold">Losowe wyniki</span>
                </div>
              </button>

              <button 
                onClick={() => { setSelectedItem(null); setCreateType('note'); setView('create'); setShowTypeSelector(false); }}
                className="parchment-card p-8 flex flex-col items-center gap-4 hover:border-primary transition-all group bg-primary/5"
              >
                <div className="bg-primary/10 p-4 rounded-full group-hover:bg-primary/20 transition-colors">
                ...
                <div className="text-center">
                  <span className="block font-black uppercase tracking-widest text-sm text-primary">Notatka</span>
                  <span className="text-[10px] text-primary/40 uppercase font-bold">Zapiski i obrazy</span>
                </div>
              </button>

              <button 
                onClick={() => { setSelectedItem(null); setCreateType('character'); setView('create'); setShowTypeSelector(false); }}
                className="parchment-card p-8 flex flex-col items-center gap-4 hover:border-primary transition-all group bg-primary/5"
              >
                <div className="bg-primary/10 p-4 rounded-full group-hover:bg-primary/20 transition-colors">
                ...
                <div className="text-center">
                  <span className="block font-black uppercase tracking-widest text-sm text-primary">Postać</span>
                  <span className="text-[10px] text-primary/40 uppercase font-bold">Karta bohatera</span>
                </div>
              </button>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-background/80 backdrop-blur-sm p-4">
          <div className="parchment-card max-w-md w-full p-8 space-y-6 shadow-2xl border-2 border-destructive animate-in zoom-in-95 duration-200">
            <div className="flex flex-col items-center text-center space-y-4">
              ...
              <h2 className="text-xl font-black uppercase tracking-tight text-primary">Potwierdź Usunięcie</h2>
              <p className="text-sm text-primary/60 italic">
                Czy na pewno chcesz usunąć te dane? Ta operacja jest nieodwracalna i zostanie odnotowana w kronikach Imperium.
              </p>
            </div>
