...
import { Trash2, Edit, Copy, Check, X, Skull, User as UserIcon, CopyPlus, FileText, Users, Table as TableIcon, Dice5 } from 'lucide-react';
...
export function GeneratorList({ generators, allTables, onEdit, onDelete, currentUser }: GeneratorListProps) {
  ...
  const formatResultForClipboard = (res: any, indent = 0): string => {
    ...
    if (res.text !== undefined && res.nested !== undefined) {
      ...
      return `${ownText}${nestedTitle}\n${formatResultForClipboard(res.nested, indent + 1)}`.trim();
    }
  ...
  const ResultRenderer = ({ data, isNested = false }: { data: any; isNested?: boolean }) => {
    if (typeof data === 'string') {
      return (
        <span className={`${isNested ? 'text-sm' : 'text-base'} text-primary font-serif italic whitespace-pre-wrap`}>
          {data}
        </span>
      );
    }
              ...
              <div
                ...
                onClick={() => {
                  ...
                  if (item.type === 'table') {
                    ...
                    setResults((prev) => [
                      { type: 'note', name: item.name, content: item.blocks[0]?.content || '' },
                      ...prev,
                    ].slice(0, 50));
                    setLastRollCount(1);
                  } else if (item.type === 'character') {
                    setResults((prev) => [
                      { type: 'character', name: item.name, race: item.race, profession: item.profession },
                      ...prev,
                    ].slice(0, 50));
                    setLastRollCount(1);
                  }
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {(previewItem as CharacterArchive).skills.map((s, i) => (
                            <span key={i} className="px-2 py-1 bg-primary/5 border border-primary/10 rounded text-xs font-bold text-primary/70">
                              {s}
                            </span>
                          ))}
                        </div>
